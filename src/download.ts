...
export async function resolveRedirects(url: string, signal?: AbortSignal): Promise<string> {
  let current = url;
...
export async function streamToFile(
  ...
  await pipeline(
    Readable.fromWeb(response.body.pipeThrough(counter) as WebReadableStream),
 ...
 */
export function rewriteRednoteHost(url: string): string {
  const parsed = new URL(url);
  if (!/(^|\.)rednote\.com$/i.test(parsed.hostname)) return url;
 ...
 */
export function recoverXhsBotWallRedirect(target: string): string {
  if (!new URL(target).pathname.startsWith('/404')) return target;
...
const YT_DLP_INSTALL_HINT =
  'Install it with `brew install yt-dlp` (macOS), `pipx install yt-dlp`, or see https://github.com/yt-dlp/yt-dlp#installation';
...
async function downloadWithYtDlp(
  ...
  if (!ytDlp) throw new MissingBinaryError('yt-dlp', YT_DLP_INSTALL_HINT);
  ...
  try {
    ...
    const staleHint =
      cause instanceof ExecError && /403|Forbidden|Unable to extract|nsig|Sign in to confirm/i.test(cause.stderrTail)
  ...
  if (filePath) return { filePath, title };
  ...
  if (!media) throw new DownloadError(`yt-dlp reported success but produced no file for ${url}`);
    ...
    if (isXhsShortLinkHost(new URL(target).hostname)) {
      ...
      if (!isXhsNoteUrl(target)) {
        throw new DownloadError(
          `Xiaohongshu short link ${url} appears to be expired or invalid (it redirected to ${target} rather than a note page). ` +
