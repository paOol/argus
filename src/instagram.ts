 ...
 */
export function buildRelayProviderVariables(providers: string[]): Record<string, boolean> {
 ...
 * Exported separately so it can be unit-tested without network access.
 */
export function extractDocIdFromBundle(bundleJs: string, queryName: string): string | null {
  const registration = new RegExp(
    `__d\\("${queryName}_instagramRelayOperation",\\[\\],\\(function\\([^)]*\\)\\{[^}]*?exports="(\\d+)"`,
  );
 ...
 * Exported separately so it can be unit-tested without network access.
 */
export function extractRelayProvidersFromBundle(bundleJs: string, queryName: string): string[] {
 ...
 * Exported separately so it can be unit-tested without network access.
 */
export function extractScriptUrlsFromHtml(html: string): string[] {
 ...
 * Exported separately so it can be unit-tested without network access.
 */
export function pickDashAudioUrl(dashManifestXml: string): string | null {
  const audioSet = dashManifestXml.match(
...
/** One media entry from the web_info payload (a post, or one carousel slide). */
interface MediaItem {
  media_type?: number;
...
function pickVideoFromItem(item: MediaItem): { videoUrl: string; isAudioOnly: boolean } | null {
  if (item.video_dash_manifest) {
    const audioUrl = pickDashAudioUrl(item.video_dash_manifest);
    if (audioUrl) return { videoUrl: audioUrl, isAudioOnly: true };
  }
 ...
 * Exported separately so it can be unit-tested without network access.
 */
export function extractInstagramVideoFromWebInfo(json: unknown): InstagramVideo | null {
  const root = json as Record<string, any>;
  const item: MediaItem | undefined = root?.data?.xdt_api__v1__media__shortcode__web_info?.items?.[0];
  if (!item) return null;

  const candidates: MediaItem[] = [item, ...(item.carousel_media ?? [])];
  for (const candidate of candidates) {
    if (candidate.media_type !== MEDIA_TYPE_VIDEO && candidate !== item) continue;
    const video = pickVideoFromItem(candidate);
    if (video) {
      const caption = item.caption?.text;
      const title =
        typeof caption === 'string' ? caption.replace(/\s+/g, ' ').trim() || undefined : undefined;
 ...
 * Exported separately so it can be unit-tested without network access.
 */
export function extractInstagramImagesFromWebInfo(json: unknown): string[] {
  const root = json as Record<string, any>;
  const item: MediaItem | undefined = root?.data?.xdt_api__v1__media__shortcode__web_info?.items?.[0];
  if (!item) return [];
  ...
  for (const slide of slides) {
    if (slide.media_type !== MEDIA_TYPE_PHOTO) continue;
...
let memoryDescriptor: QueryDescriptor | null = null;

async function loadCachedDescriptor(): Promise<QueryDescriptor | null> {
...
async function saveCachedDescriptor(descriptor: QueryDescriptor): Promise<void> {
...
async function fetchText(url: string, accept: string, signal: AbortSignal | undefined): Promise<string> {
 ...
 */
async function harvestQueryDescriptor(
  shortcode: string,
  ...
  if (scriptUrls.length === 0) {
    throw new DownloadError(
      `Could not find any script bundles on ${embedUrl} to recover Instagram's current query id from.`,
    );
  }

  const bundles = await Promise.all(
    scriptUrls.map((u) => fetchText(u, '*/*', signal).catch(() => '')),
  );
  ...
  let relayProviders: string[] | null = null;
  for (const bundle of bundles) {
    ...
    if (providers.length > 0) relayProviders ??= providers;
  }
  if (docId) return { docId, relayProviders: relayProviders ?? DEFAULT_RELAY_PROVIDERS };
  throw new DownloadError(
    `Instagram's ${POST_QUERY_NAME} query was not found in any of the ${scriptUrls.length} script bundles ` +
...
async function resolveShareRedirect(url: string, signal: AbortSignal | undefined): Promise<string> {
  let current = url;
...
export async function resolveInstagramVideo(
  ...
  if (!post) {
    throw new DownloadError(
      `Not an Instagram post URL: ${url}. Expected something like https://www.instagram.com/reel/<code>/ ` +
        '(stories and profile links are not supported).',
    );
  }

  const cached = await loadCachedDescriptor();
  const defaults: QueryDescriptor = { docId: DEFAULT_DOC_ID, relayProviders: DEFAULT_RELAY_PROVIDERS };
  const tried = new Set<string>();
  let postWithoutVideo: unknown;

  const attempt = async (descriptor: QueryDescriptor): Promise<InstagramVideo | null> => {
    if (tried.has(descriptor.docId)) return null;
    ...
    const items = (json as Record<string, any>)?.data?.xdt_api__v1__media__shortcode__web_info?.items;
    if (Array.isArray(items) && items.length > 0) postWithoutVideo = json;
    return null;
  };
  ...
  if (postWithoutVideo) {
    ...
    throw new NoVideoError(
      imageUrls.length > 0
        ...
        : `No video found in Instagram post ${url}.`,
      imageUrls,
    );
  }
  throw new DownloadError(
    `Instagram post ${url} is unavailable anonymously (deleted, private, or login-gated). ` +
