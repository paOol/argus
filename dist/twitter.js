 ...
 */
export function parseTwitterStatusUrl(url) {
    let parsed;
    try {
        parsed = new URL(url);
    }
 ...
 */
export function buildSyndicationToken(id) {
 ...
 * Exported separately so it can be unit-tested without network access.
 */
export function extractTwitterVideoFromSyndication(json) {
 ...
 * Exported separately so it can be unit-tested without network access.
 */
export function extractTwitterImagesFromSyndication(json) {
    const root = json;
 ...
 * Exported separately so it can be unit-tested without network access.
 */
export function pickAudioRenditionUrl(masterM3u8, baseUrl) {
    let best = null;
    for (const line of masterM3u8.split('\n')) {
        if (!/^#EXT-X-MEDIA:/.test(line) || !/\bTYPE=AUDIO\b/.test(line))
            continue;
 ...
 */
export async function resolveTwitterVideo(url, options = {}) {
    const status = parseTwitterStatusUrl(url);
    if (!status) {
        throw new DownloadError(`Not a tweet URL: ${url}. Expected something like https://x.com/<user>/status/<id>.`);
    }
    const signals = [];
    if (options.signal)
    ...
    if (response.status === 404) {
        throw new DownloadError(`Tweet ${status.id} is unavailable (deleted, protected, or age-restricted). ` +
    ...
    if (!video) {
        ...
        throw new NoVideoError(imageUrls.length > 0
            ...
            : `No video found in tweet ${url}. ` +
                'The tweet may contain only text, or its video may be an external link (these are not supported).', imageUrls);
    }
    if (!video.isHls)
        return video;
    ...
    try {
        ...
        if (master.ok) {
            ...
            if (audioUrl)
                return { ...video, videoUrl: audioUrl };
        }
        else {
