...
import type { ProgressEvent, TranscribeOptions } from './types.js';
...
export function parseArgs(argv: string[]): CliFlags {
  const flags: CliFlags = { format: 'text', keepAudio: false, quiet: false, doctor: false, help: false };

  const takeValue = (name: string, next: string | undefined): string => {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    switch (arg) {
      ...
      case '--keep-audio': flags.keepAudio = true; break;
      case '-m': case '--model': flags.model = takeValue(arg, argv[++i]); break;
      case '-l': case '--language': flags.language = takeValue(arg, argv[++i]); break;
      case '-o': case '--output': flags.output = takeValue(arg, argv[++i]); break;
      ...
      case '--whisper': flags.whisper = takeValue(arg, argv[++i]); break;
      case '-f': case '--format': {
        ...
        flags.format = value as CliFlags['format'];
        break;
      }
      case 'doctor': flags.doctor = true; break;
      default:
        if (arg.startsWith('-')) throw new Error(`Unknown option ${arg}`);
        if (flags.url) throw new Error(`Unexpected extra argument "${arg}"`);
        flags.url = arg;
    }
  }
  return flags;
...
async function runDoctor(): Promise<number> {
  ...
  const rows: Array<[string, typeof report.ytDlp, string]> = [
...
export async function main(argv: string[] = process.argv.slice(2)): Promise<number> {
  ...
  if (flags.model && !flags.model.includes('/') && !flags.model.endsWith('.bin') && !(KNOWN_MODELS as readonly string[]).includes(flags.model)) {
  ...
  try {
    ...
    if (!flags.quiet && process.stderr.isTTY) process.stderr.write('\r\x1b[2K');

    let rendered: string;
    switch (flags.format) {
      case 'json': rendered = JSON.stringify(result, null, 2); break;
      case 'srt': rendered = toSrt(result.segments); break;
      case 'vtt': rendered = toVtt(result.segments); break;
      case 'timestamps': rendered = toTimestampedText(result.segments); break;
      default: rendered = result.text;
    }

    if (flags.output) {
      await writeFile(flags.output, rendered + '\n', 'utf8');
      if (!flags.quiet) console.error(`Wrote ${flags.format} transcript to ${flags.output}`);
    } else {
      console.log(rendered);
    }
    if (result.audioPath) console.error(`Kept audio at ${result.audioPath}`);
    return 0;
  } catch (error) {
    if (!flags.quiet && process.stderr.isTTY) process.stderr.write('\r\x1b[2K');
    ...
    if (error instanceof NoVideoError && error.imageUrls.length > 0) {
      ...
      for (const imageUrl of error.imageUrls) console.log(imageUrl);
      return 3;
    }
