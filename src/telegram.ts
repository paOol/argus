 ...
 */
export function parseTelegramUrl(url: string): TelegramPost {
  ...
  if (segments[0] === 'c') {
    throw new UnsupportedUrlError(
      url,
      'private Telegram links (t.me/c/...) have no public embed; download the video with a Telegram client and use transcribeFile()',
    );
  }
 ...
 */
export function extractVideoUrlFromEmbedHtml(html: string): string | null {
...
): Promise<{ videoUrl: string; post: TelegramPost }> {
