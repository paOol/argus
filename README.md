...
When a link points at an image post instead of a video (an Instagram photo post, a Reddit image/gallery post, a photo tweet, or a Telegram photo post), `transcribe` throws `NoVideoError` (code `NO_VIDEO`) with the direct image URL(s) attached, so callers can handle the picture some other way:
